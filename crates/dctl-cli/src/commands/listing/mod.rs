...
/// Say on stderr what the listing's walk passed over.
///
...
/// 0. The special files are here for the same reason and were found the same
/// way: `dctl ls` over a directory holding a file and a named pipe printed the
/// file and said nothing whatever about the pipe.
///
...
pub fn report_omissions(ctx: &Ctx, stream: &Stream) {
    crate::links::report(ctx, &stream.links());
    crate::specials::report(ctx, &stream.specials());
}
