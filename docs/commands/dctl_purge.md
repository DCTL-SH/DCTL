...
| Code | Name | When |
|------|------|------|
| 1 | `usage` | Unparseable command line; a local, UNC, malformed-remote, empty or `..`-containing target; **a run with neither `--force` nor `--interactive` nor `--dry-run`**; `--interactive` with no terminal to prompt on. |
