...
- **The audit log is unkeyed, and `audit-key-v1` is not what keys it.** The sub-key label
  ...
  this build consumes it**: `docs/AUDIT_LOG.md` §3's chain hash is plain, unkeyed BLAKE3
  ...
  **a valid chain proves integrity and order — and, with an anchor, length — but never
  ...
  records to any particular writer. `docs/AUDIT_LOG.md` §11 carries the argument for why a
  ...
  exposure. A future keyed or signed profile is a **new** record version under `AUDIT_LOG.md` §2.1's
